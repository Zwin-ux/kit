...
| File | Freezes |
|---|---|
| `kit-core/src/run.rs` | `Run`, `RunState`, `RunSpec`, `Bounds`, `RunDelta`, `Receipt` |
| `kit-core/src/config.rs` | `kit.toml` schema |
| `kit-core/src/gate.rs` | `Gate` trait, `GateOutcome`, `GateCheck`, `FirewallVerdict` |
| `kit-agents/src/lib.rs` | `Agent` trait, `AgentHandle`, `AgentStatus` — the adapter seam |
| `kit-tui/src/event.rs` | `AppEvent` enum, `Clock`, tick cadence |

The gate *contract* lives in `kit-core`, not `kit-gate`: `Run` and `Receipt` both
carry a `GateOutcome`, so putting the type in `kit-gate` would make the dependency
circular. `kit-gate` holds the engine and implements `kit_core::Gate`.
