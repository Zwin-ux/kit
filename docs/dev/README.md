# Contributor / engineering docs

These notes support people changing Kit itself. End users can ignore this folder.

| Doc | Topic |
|-----|--------|
| [AGENTS.md](AGENTS.md) | Context for coding agents working in this repo |
| [ARCHITECTURE.md](ARCHITECTURE.md) | Package layout and data flow |
| [ROADMAP.md](ROADMAP.md) | What’s next |
| [SKILL_SCHEMA.md](SKILL_SCHEMA.md) | SKILL.md front matter rules |
| [HARNESS_PATHS.md](HARNESS_PATHS.md) | Where skills link into each agent |
| [LOCAL_LIBRARY.md](LOCAL_LIBRARY.md) | Offline library layout |
| [TUI_SCREENS.md](TUI_SCREENS.md) | Pixel TUI screens and keys |
| [PACKAGE_STRUCTURE.md](PACKAGE_STRUCTURE.md) | Monorepo map |
| [REGISTRY_API.md](REGISTRY_API.md) | Catalog / Railway service |
| [TESTING.md](TESTING.md) | Test commands and gates |
| [PIXEL_ART.md](PIXEL_ART.md) | Mascot and marketing art rules |
| [LAUNCH-PRESENTATION.md](LAUNCH-PRESENTATION.md) | GitHub Page / presentation — **must use [scroll-world](https://github.com/oso95/scroll-world)** |

Public product docs: [../README.md](../README.md) · [../packs.md](../packs.md)
